...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (existingUser) {
      return res.status(400).json({ error: { code: "DUPLICATE", message: "Username or Email already exists" } });
    }
    ...
    const accessToken = signAccessToken({ sub: user._id.toString(), roles: user.isAdmin ? ["admin"] : ["user"] });
    ...
    res
      ...
      .status(201)
      .json({
        user: { id: user._id, username: user.username, email: user.email, profilePicture: user.profilePicture, isAdmin: user.isAdmin },
        accessToken,
      });
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!validPassword) return res.status(401).json({ error: { code: "INVALID_CREDENTIALS", message: "Wrong password" } });
